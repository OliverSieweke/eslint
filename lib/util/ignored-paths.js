...
class IgnoredPaths {
    ...
    contains(filepath, category) {
        ...
        /*
         * `ignore` package checks the trailing slash to distinguish if a path
         * is a file or a directory.
         */
        const endsWithSlash = filepath.endsWith("/") || filepath.endsWith("\\");
        const normalizedPath = relativePath + (endsWithSlash ? path.sep : "");

        if (relativePath) {
            if (typeof category === "undefined") {
                result =
                    this.ig.default.ignores(normalizedPath) ||
                    this.ig.custom.ignores(normalizedPath);
            } else {
                result =
                    this.ig[category].ignores(normalizedPath);
            }
        }
        debug("contains:");
        debug("  target   = %j", filepath);
        debug("  base     = %j", basePath);
        debug("  relative = %j", normalizedPath);
